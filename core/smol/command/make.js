...
module.exports = {
  ...
  exec: async command => {
    ...
    for (let file of make.files) {
      ...
      if (file.parse) fileContents = file.parse({content: fileContents, filename: command.args.filename, isExample: command.args.example, core: command.args.core})
      let relativeTarget = file.to(command.args.filename, command.args.core)
